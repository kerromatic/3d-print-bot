...
async def get_snapshot_interval():
    from bot.scheduler import get_snapshot_interval
    return {"interval": get_snapshot_interval()}

@app.post("/api/snapshot-interval")
async def set_snapshot_interval(interval: int = Form(...)):
    from bot.scheduler import set_snapshot_interval
    interval = max(60, interval)
    set_snapshot_interval(interval)
