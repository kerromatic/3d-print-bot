"""Scheduled tasks ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ Print of the Day, Tip of the Day, auto-gallery."""
...
async def run_gallery_scan(context: ContextTypes.DEFAULT_TYPE):
    ...
    for img_path in new_images:
        ...
        if buf:
            ...
            await post_to_gallery(context.bot, buf, caption=f"ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ¸ {filename}")
...
"""Scheduled tasks ÃÂ¢ÃÂÃÂ Print of the Day, Tip of the Day, auto-gallery."""
...
async def run_gallery_scan(context: ContextTypes.DEFAULT_TYPE):
    ...
    for img_path in new_images:
        ...
        if buf:
            filename = Path(img_path).stem.replace("_", " ").replace("-", " ").title()
            await post_to_gallery(context.bot, buf, caption=f"ÃÂ°ÃÂÃÂÃÂ¸ {filename}")
            mark_as_posted(folder, Path(img_path).name)
...
async def run_cam_snapshot(context: ContextTypes.DEFAULT_TYPE):
    """Capture and post a snapshot - only when actively printing."""
    if not settings.PRINTER_IP or not settings.PRINTER_ACCESS_CODE:
        return
    if not settings.TOPIC_LIVECAM:
        return
