"""Channel posting logic — routes content to the correct topics in the supergroup."""
...
async def post_new_print(bot: Bot, print_data: dict, image=None) -> int | None:
    ...
    if photo:
        ...
        msg = await bot.send_photo(
            ...
            message_thread_id=settings.TOPIC_ANNOUNCEMENTS,
...
async def post_to_gallery(bot: Bot, image, caption: str = "") -> int | None:
    ...
    msg = await bot.send_photo(
        chat_id=settings.MAIN_GROUP,
        message_thread_id=settings.TOPIC_GALLERY,
        photo=photo, caption=caption, parse_mode="HTML",
    )
    return msg.message_id
...
async def post_request(bot: Bot, request: dict) -> int | None:
    ...
    msg = await bot.send_message(
        chat_id=settings.MAIN_GROUP,
        message_thread_id=settings.TOPIC_REQUESTS,
        text=text, parse_mode="HTML",
    )
    return msg.message_id
...
async def post_tip(bot: Bot, tip: dict) -> int | None:
    ...
    msg = await bot.send_message(
        chat_id=settings.MAIN_GROUP,
        message_thread_id=settings.TOPIC_TIPS,
        text=text, parse_mode="HTML",
    )
    return msg.message_id
